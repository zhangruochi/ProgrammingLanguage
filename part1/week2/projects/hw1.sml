...
and returns the number of dates in the list of dates that are in any of the months in the 
list of months. Assume the list of months has no number repeated. Hint: Use your answer to the previous problem.*)

fun number_in_months( dates: (int*int*int) list, list_month: int list) = 
    if null list_month
    then 0
    else number_in_month(dates, hd list_month) + number_in_months(dates, tl list_month)
...
has no number repeated. Hint: Use your answer to the previous problem and SML’s list-append 
operator (@).*)
...
st. Do not worry about the case where the list has too few elements: list where the head of the list is 1
your function may apply hd or tl to the empty list in this case, which is okay.*)


fun get_nth(s: string list, nth: int) = 
    if nth = 1
    then hd s
    else get_nth(tl s, nth - 1)


(*7. Write a function date_to_string that takes a date and returns a string of the form January 20, 2013 
(for example). Use the operator ^ for concatenating strings and the library function Int.toString 
for converting an int to a string. For producing the month part, do not use a bunch of 
conditionals. Instead, use a list holding 12 strings and your answer to the previous problem. 
For consistency, put a comma following the day and use capitalized English month names: 
January, February, March, April, May, June, July, August, September, October, November, December.*)

fun date_to_string(date: int*int*int) = 
    let
        val month_str = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
    in
    get_nth(month_str, #2 date) ^ " " ^ Int.toString(#3 date) ^ ", " ^ Int.toString(#1 date)
    end
...
fun number_before_reaching_sum(sum: int, numbers: int list) = 
    ...
    let fun cal(cur_sum: int, numbers: int list, count: int) = 
        if cur_sum + (hd numbers) >= sum
        then count
        else cal(cur_sum + (hd numbers), tl numbers, count+1)
    in
        cal(0, numbers, 0)
    end

(*9. Write a function what_month that takes a day of year (i.e., an int between 1 and 365) and returns what month 
that day is in (1 for January, 2 for February, etc.). Use a list holding 12 integers and your 
answer to the previous problem.*)


fun what_month(date: int) = 
    let 
        val num_day_in_month = [31,28,31,30,31,30,31,31,30,31,30,31]
    in 
        number_before_reaching_sum(date, num_day_in_month) + 1
    end


(*10. Write a function month_range that takes two days of the year day1 and day2 
and returns an int list [m1,m2,...,mn] where m1 is the month of day1, m2 is the month 
...
fun oldest (dates : (int * int * int) list) =
    if null dates then NONE
        ...
        let fun recurse(dates: (int * int * int) list) =
            if null (tl dates) then hd dates
            else
               let val tail_oldest = recurse(tl dates)
               in
...
fun remove_dup(nums: int list) = 
    if null nums then []
        ...
        let fun filter_list (unique : int, remaining_list: int list) =
            if null remaining_list then []
            else 
                let val filtered_remaining_list = filter_list(unique, tl remaining_list)
                in
                    if hd remaining_list = unique then filtered_remaining_list
                    else hd remaining_list :: filtered_remaining_list
                end
        in
            hd nums :: remove_dup(filter_list(hd nums, tl nums))
        end
